...
def clean_name(name):
    """Remove common prefixes like 'Dr.', trim spaces, and lowercase the name."""
    return re.sub(r"^(dr\\.|mr\\.|ms\\.|mrs\\.)\\s*", "", name, flags=re.IGNORECASE).strip().lower()

@app.route("/search", methods=["GET"])
def search_attendee():
    first_name = clean_name(request.args.get("first_name", ""))
    middle_name = clean_name(request.args.get("middle_name", ""))
    last_name = clean_name(request.args.get("last_name", ""))
    ...
    if not col_first_name or not col_last_name or not col_submission_id:
    ...
    for attendee in attendees:
        ...
        stored_first_name = clean_name(attendee.get(col_first_name, ""))
        stored_middle_name = clean_name(attendee.get(col_middle_name, ""))
        stored_last_name = clean_name(attendee.get(col_last_name, ""))

        # Flexible matching for first, middle, and last names
        if (
            first_name in stored_first_name
            and (not middle_name or middle_name in stored_middle_name or middle_name[0] == stored_middle_name[0])
            and last_name in stored_last_name
        ):
            ...
            passport_url = attendee.get(col_passport, "").strip() if stored_submission_id else None
            flight_details_url = attendee.get(col_flight_details, "").strip() if stored_submission_id else None
